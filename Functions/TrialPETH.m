function [FinalData TrialData] = TrialPETH( Fs, Ts, Pre, Post, EventTS,Ch490,Ch405, varargin)
...
%% %% %% %% %% %% %% 
 ...
 %NEED TO REMOVE -1 IF POSSIBLE!!!
    ...
    for i=1:length(EventTS)
        [MinVal, CSidx(i,1)]=min(abs(Ts(:,1)-EventTS(i)));
    end
...
set(gca,'fontsize',18);
if length(varargin)>=1
    FastPrint([varargin{1} 'Z-score'])
else
    FastPrint('CS_Photometry-ZScore')
end
...
set(gca,'fontsize',18);
if length(varargin)>=1
    FastPrint(varargin{1})
else
    FastPrint('CS_Photometry-DFF')
end
